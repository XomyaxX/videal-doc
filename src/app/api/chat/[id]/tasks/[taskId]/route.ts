...
import { assertChatWriter, serializeTask } from "@/lib/chat-widgets";
...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string; taskId: string }> }) {
  ...
  const task = await prisma.chatTask.findFirst({ where: { id: taskId, chatId: id } });
  ...
  if (task.authorId !== session.user.id && task.assigneeId !== session.user.id) {
    return NextResponse.json({ error: "Это не ваша задача" }, { status: 403 });
  }
